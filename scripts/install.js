...
async function install() {
    ...
    // Add cost-tracker hook for real-time data
    if (!settings.hooks) settings.hooks = {};
    if (!settings.hooks.Stop) settings.hooks.Stop = [];

    const costTrackerCommand = `node ${PLUGIN_INSTALL_PATH.replace(/\\/g, '/')}dist/hook-handler.js --hook=cost-tracker \"${HOOK_DATA}\"`;
    const hasCostTracker = settings.hooks.Stop.some(
        hook => hook.command && hook.command.includes('hook-handler')
    );
    if (!hasCostTracker) {
